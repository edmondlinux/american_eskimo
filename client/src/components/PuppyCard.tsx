...
export function PuppyCard({
  ...
  return (
    <div
      className={cn(
        "surface-card grain-overlay group rounded-3xl border border-border/70 p-0 overflow-hidden flex flex-col h-full",
        "transition-all duration-300 hover:-translate-y-1 hover:shadow-[var(--shadow-lift)]",
        className
      )}
      data-testid={`puppy-card-${puppy.id}`}
    >
      <div className="aspect-[4/3] relative w-full overflow-hidden bg-secondary/20">
        {puppy.imageUrl ? (
          <img 
            src={puppy.imageUrl} 
            alt={puppy.name}
